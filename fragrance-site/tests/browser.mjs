...
try {
  ...
  await prepare(page);
  check(await page.title() === 'FABREVOIE — ULTRA MACHO · The pleasure is yours.', 'Correct brand and headline in title');
  check((await page.locator('h1').innerText()).replace(/\s+/g, ' ').trim() === 'ULTRA MACHO', 'Product name is the primary heading');
  check(await page.locator('.hero-slogan').innerText() === 'The pleasure is yours.', 'Approved slogan supports the product heading');
