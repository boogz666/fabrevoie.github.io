...
**Current activation dependency:** `DATABASE_URL` has not been configured. Vercel requires the account holder to accept the Neon marketplace terms before provisioning the intended free database. The acceptance link is https://vercel.com/puppetmaster666s-projects/~/integrations/accept-terms/neon?source=cli. No paid database plan has been selected.

While storage setup is pending, the production build displays a release-list opening notice instead of the signup form. The build automatically enables the form when `DATABASE_URL` and `RATE_LIMIT_SECRET` are present. Without storage, the API returns 503 and never reports a saved signup. After connecting the database, verify a disposable signup and withdrawal before announcing that the list is open.
