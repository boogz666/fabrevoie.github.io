...
Target repository: **https://github.com/boogz666/fabrevoie.github.io**, branch **`feat/official-waitlist-2026`**. This `website/` directory is the working source. The deployable application is packaged under **`fragrance-site/`** in that repository, preserving the original root sneaker source for rollback. Earlier launch branches preserve the preceding design. GitHub publishing still requires write access to the target account; the currently connected `puppetmaster666` account has read-only access. The previous `puppetmaster666/fabrevoie` repository is retained as a historical project; it is not the target for this update.

Vercel project: **https://vercel.com/puppetmaster666s-projects/fabrevoie**. Deployment address: **https://fabrevoie.vercel.app**. Both **fabrevoie.com** and **www.fabrevoie.com** are attached to this project; www redirects to the apex. Production canonical metadata uses **https://fabrevoie.com**. The domain still needs the GoDaddy DNS records in [OFFICIAL-LAUNCH.md](OFFICIAL-LAUNCH.md) applied. Legacy sneaker HTML routes redirect temporarily to the perfume homepage on Vercel. The current workflow is a manual deployment after verification.
